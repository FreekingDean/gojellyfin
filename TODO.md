...
- `GetUtcTime` reads the reception time inside the handler; request details like that belong on the context for observability (#426).
...
- `UpdateUserPassword` and `cmd/tasks/resetpassword` leave the user's existing sessions alive; `sessions` has no revoke-by-user and both should use it.
- `ForgotPassword` can only answer `ContactAdmin` until the server can reach an account holder off-box; a real pin flow needs mail (or another out-of-band channel) first, and `Action` is already `InNetworkRequired`/`PinCode` in the spec for when it does.
