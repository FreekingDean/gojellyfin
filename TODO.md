...
- `Playlist` has no owner column, so nothing distinguishes the creator from a share and no playlist handler can enforce the 403s the spec documents; `PlaylistEntry` also leaves its `playlist`/`item` foreign keys unexported, which costs an eager load for every entry read.
- `AddItemToPlaylist` adds exactly the ids it is given; Jellyfin expands a folder id (album, series) into its children first.
