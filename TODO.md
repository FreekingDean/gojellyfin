...
- An administrator can post `IsStartupWizardCompleted: false` to `POST /System/Configuration` and re-open the unauthenticated wizard; `configuration.UpdateConfiguration` stores the body verbatim, so the flag needs to be owned by `startup` and ignored on that path.
- `users.EnsureUser` keys on the submitted username, so walking the wizard back and finishing under a different name leaves the first administrator behind with a known password; upstream renames the first user instead, which needs `Rename` to move `username` as well as `name`.
