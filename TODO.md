...
- The seven `Startup` operations stay 501 by decision: there is no setup wizard, the server assumes it is already set up, and `DefaultServerConfiguration` says so with `IsStartupWizardCompleted: true` so clients skip it. `cmd/tasks/adduser` creates the first user. Implementing them means an unauthenticated path gated on mutable state, which is not worth the surface.
