...
`charts/gojellyfin` is how that image is deployed — the API, the streaming pods, the worker and the `jellyfin-web` nginx, all from the one tag. It holds no credential and owns no storage: `DATABASE_URL` and `TMDB_API_KEY` are `secretKeyRef`s into Secrets the operator creates, and the media is a volume source handed to `media.volume` rather than a claim the chart writes. `TRANSCODER_JOBS` follows the workload's cpu request rounded down, so the encode slots move with the cpu instead of being a second number to keep in step. `MIGRATE_ON_START` is not exposed at all; migration is a one-off run, or the `pre-install,pre-upgrade` hook Job that a release carrying a migration turns on.
