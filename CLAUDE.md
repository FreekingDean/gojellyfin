...
**A job names its own task queue.** `Job.Queue()` is what `jobs.Service.Start` puts on the run and what `Worker.Start` groups the registry by, so the worker builds one `worker.New` per distinct queue rather than one for everything. `LibraryScan` polls `gojellyfin` and `Identify` polls `gojellyfin-metadata`, because the two are bounded by different things — a probe saturates a core and a metadata fetch waits on a rate-limited API — and a fan out of one behind a shared cap starves the other. A child workflow and an activity inherit the queue of the workflow that started them, so `ProbeChunk` and `ProbeSource` need none of their own: the SDK seeds a workflow's root context with its own queue and `WithChildOptions`/`WithActivityOptions` override only a queue that was named. Both queues take the same worker options today — the point of the split is that a queue can be tuned without touching the other, not that it already has been. Renaming one strands what is already scheduled on the old name: nothing polls it after the deploy, so it fails on its `ScheduleToStartTimeout` and the next run re-derives the work from the rows.
