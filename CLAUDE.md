...
**A client is handed one url and no choice of path.** `PlaybackInfo` answers every video source with `SupportsDirectPlay` and `SupportsDirectStream` false, `SupportsTranscoding` true, and a `TranscodingUrl` this server generated, so `jellyfin-web` never builds a stream url of its own and never picks between two of them. Where that url leads is decided from the `DeviceProfile` the client posted: `mediainfo.directPlays` reads its `DirectPlayProfiles`, and where one covers the source's container and its first audio track the url names the source's own container and codec and the handler serves the file; where none does, `mediainfo.remuxContainer` intersects what the client declared with the containers `internal/transcode` can write down a pipe and the url names that. Container and audio codec only, the same pair `items.PreferredSource` matches on; video codec stays out (#532, #481). A client that posts no profile is handed its source untouched, because guessing a container is worse than leaving one alone.

Both ride in the query, so `internal/server/stream` reaches the same answer from the request alone. That is why the decision lives in the url rather than on the session: the capability data only ever arrives on the `PlaybackInfo` body, and `jellyfin-web` posts no `DeviceProfile` with `Sessions/Capabilities/Full` (#584), so a stream request has nothing else to read.
...
`UserPolicy.EnableVideoPlaybackTranscoding` is answered false for the same reason: the picture is copied and never re-encoded (#481), and that policy is what `jellyfin-web` reads to decide whether to offer a bitrate menu. `MaxStreamingBitrate` is not honoured, so the menu would change nothing (#585).
