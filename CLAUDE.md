...
**A child resolves its parent by key.** `RefreshItem` for an episode looks up `season:tmdb:1396:1` and fails if it is not there yet, which the activity's bounded retry covers — the parent was enqueued first, and a run that still loses the race leaves that one episode for the next refresh rather than writing it parentless.
