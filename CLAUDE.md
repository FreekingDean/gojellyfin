...
**A client is handed one source and one url, and neither is its choice.** `PlaybackInfo` answers with exactly one `MediaSourceInfo` — `SupportsDirectPlay` and `SupportsDirectStream` false, `SupportsTranscoding` true, and a `TranscodingUrl` this server generated — or with no sources at all and `ErrorCode: NoCompatibleStream`. The client never learns a second file exists, which is why the version picker in `jellyfin-web` no longer has anything to pick from: an item's 4K and 1080p copies are one answer now, not two.

**Which file that is belongs to `internal/items`, not to the tag package.** `items.SourceFor(ctx, itemID, items.Capabilities)` is the whole question — of the files behind this item, which one, and what has to change about it — and `items.Plan` is the answer. `internal/server/mediainfo` does three things and nothing else: `capabilities` reads the posted `DeviceProfile` into `items.Capabilities`, `SourceFor` is called, and `mediaSourceDto`/`served`/`streamURL` translate the plan back. The domain cannot see `api`, so a device profile has to become a domain type at the boundary; `items.Profile` is one line of one, a container beside the codecs it may carry.

A client says what it decodes in two places and both are read. A `DirectPlayProfile` is a triple — container, video codec, audio codec — and the `CodecProfiles` beside it carry the conditions: Chrome names `h264` and then says it wants `SDR`, a `VideoProfile` it knows, a `VideoLevel` it can reach, and neither interlacing nor anamorphic pixels. Reading only the first was the AC-3 bug again in the picture, since an HDR remux passes the codec check and plays washed out with nothing to say so. **Which of those fails is what decides the cost**, and `items.Capabilities.planFor` answers it as a ladder, cheapest first:

1. `ChangeNone` — the container and both codecs are declared, so the url names the source's own container and codecs and the handler serves the file, byte ranges and all;
2. `ChangeContainer` — the codecs are declared and the container is not, so both streams are copied into a container the client declared that `internal/transcode` can write;
3. `ChangeAudio` — the audio codec is not declared, so the same mux with `-c:a` set to what that container carries, and the picture still copied;
4. `ChangeVideo` / `ChangeVideoAudio` — the video codec is not declared. `Change.Available()` is false for both: re-encoding a picture is #481, so these are ranked but never served, and a source that needs one is refused rather than answered with bytes nothing here can make.

**`items.Capabilities.best` is where the choosing happens, and there is no ranking in it.** A source's plan is already the least that has to change about it, so the files are walked tallest first and the first one whose plan can be served is the answer. That is Dean's ordering with none of the machinery: a 4K needing its audio converted is returned before a 1080p that plays untouched because the 4K is reached first, and a file needing a picture encode is passed over rather than ranked last. Resolution is the first video stream's `height`, which `MediaSources` already eager-loads; a file the probe has not reached has none, and tiers with the tallest rather than below everything, because "not looked at yet" is not "small". Two files at one resolution fall to `richer`, which is bitrate then size — note that at one resolution the cheaper plan no longer wins, so a fat remux beats a thin re-encode even though it costs an audio convert.

**A condition is held as hard.** Every one jellyfin-web writes carries `IsRequired: false`, and the client never evaluates them itself, so what that flag means for direct play cannot be read off the bundle — upstream uses it to decide what a transcode must enforce, not what direct play may ignore. Holding them all is the reading that cannot play the wrong bytes. Only the three verbs the client writes for video are implemented (`EqualsAny`, `NotEquals`, `LessThanEqual`); an unread verb, an unread property and an unprobed value all pass, because a file is never refused for something nobody read. The conditions can only be answered because `scanner.probe` now fills in `video_range_type`, `is_interlaced` and `is_anamorphic`, which the ent schema had carried unwritten since it was generated — a file whose transfer characteristic ffprobe does not report is left unknown rather than called SDR, since that guess is paid for by playing HDR through an SDR decoder.

Collapsing 2 into 3 is the failure worth guarding: a needless audio encode still plays, so nobody notices it except the cpu. `stream.needsRemux` asks the container question and `stream.convertsAudio` asks the audio one separately, and `transcode.Spec.CopyAudio` is what keeps rung 2 from paying for rung 3. Codec names alone cannot tell the two apart — aac re-encoded to aac is still aac — so the tests fingerprint each stream with ffmpeg's md5 muxer and compare it against the source.

**Codec names are compared as written, because both sides write them the same way.** `jellyfin-web` builds its `DirectPlayProfiles` out of `h264`, `hevc`, `mpeg2video`, `vc1`, `msmpeg4v2`, `vp8`, `vp9`, `av1`, `aac`, `mp3`, `ac3`, `eac3`, `mp2`, `pcm_s16le`, `pcm_s24le`, `truehd`, `aac_latm`, `opus`, `flac`, `alac` and `vorbis` — ffprobe's own spellings, which is what `scanner.probe` stores verbatim as `stream.CodecName`. The one codec with two names in circulation is DTS, and the client pushes `dca` and `dts` together rather than choosing, so plain comparison matches. An alias table was written and thrown away: nothing in it would ever have fired, and a guessed alias is a new way to be wrong. `TestPlanFor` pins the shared vocabulary against the strings read off the vendored bundle, so a future disagreement fails a test rather than silently changing a rung. Case and padding are folded (`strings.EqualFold`, `TrimSpace`) because profile strings are client-authored; that is a fold, not a translation.

Container, video codec and audio codec all ride in the query, so `internal/server/stream` reaches the same rung from the request alone. That is why the decision lives in the url rather than on the session: the capability data only ever arrives on the `PlaybackInfo` body, and `jellyfin-web` posts no `DeviceProfile` with `Sessions/Capabilities/Full` (#579), so a stream request has nothing else to read. `acceptedVideo` is the one of the three with no fallback guess — silence about the picture is silence, because a wrong guess there cannot be rescued by an encode.

Three things have to agree and are tested together: the suffix in the url, the `container` parameter, and the `Content-Type` of the response. A browser dispatches on the header, which is how an AC-3 rip played its picture in silence — it asked for `stream.mkv?Static=true` and `stream.needsRemux` treated `static` as a veto before it read either the container or the codec. That veto is gone; `Serve` still honours `static` where the client named a container the source is not in, which is what makes a download ignore the extension.

`SupportsTranscoding` and a non-nil `TranscodingUrl` travel together in both directions, and the url always refuses both stream copies. `jellyfin-web` answers a playback error by asking for the stream all over again and stops only once the url it was handed carries `allowvideostreamcopy=false` and `allowaudiostreamcopy=false`; claiming transcoding with no url to name left that condition unreachable and the client retried as fast as the round trip allowed (#572).
