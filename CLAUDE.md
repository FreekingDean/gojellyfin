...
A `DirectPlayProfile` is a triple, so **which of the three fails is what decides the cost**. `items.Capabilities.planFor` answers it as a ladder, cheapest first:
...
**`items.ranked` is where Dean's ordering lives**, and it is not cheapest-first: resolution dominates and the change breaks the tie, so a 4K needing its audio converted beats a 1080p that plays untouched. A picture needing an encode is the exception — it ranks below every source that does not, whatever its resolution, and only at the shortest source it applies to, because the encode discards the extra resolution anyway. Resolution is the first video stream's `height`, which `MediaSources` already eager-loads; a file the probe has not reached has none, and tiers with the tallest rather than below everything, because "not looked at yet" is not "small". Two files at one resolution and one cost fall to `richer`, which is bitrate then size.
