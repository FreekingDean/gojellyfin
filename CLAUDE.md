...
**What a client can play is answered at `PlaybackInfo`, not at the stream.** `jellyfin-web` asks for `stream.mkv?Static=true` whenever the source came back with `SupportsDirectStream`, and `middleware.HttpCanonicalQuery` folds `Static` onto the spec's `static`, so a source this server called direct playable is served untouched and the decision in `internal/server/stream` never runs — an AC-3 rip played its picture in silence in Chrome. `mediainfo.directPlays` reads the `DirectPlayProfiles` the client posted, and where none covers the source's container and its first audio track the source is answered with `SupportsDirectPlay` and `SupportsDirectStream` false and a `TranscodingUrl` at `/Videos/{id}/stream.mp4`, which is the remux. Container and audio codec only, the same pair `items.PreferredSource` matches on; video codec stays out (#532, #481). A client that posts no profile, or one whose profile names no video, is answered exactly as before — the tables in `internal/server/stream` are still what a client that declares nothing is held to.
