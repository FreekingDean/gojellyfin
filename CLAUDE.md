...
Everything ships as one cobra binary, `cmd/gojellyfin`, with a subcommand each for `server`, `worker`, `migrate`, `adduser`, `resetpassword`, `import` and `localizationdata`. One binary means one image, so an operator task is `docker exec <container> gojellyfin adduser Dean` rather than a second image or a second build. The names stay flat and read as commands; a new task is a `<name>Command() *cobra.Command` constructor in a file of its own, added to the list in `main.go`.
...
**Jellyfin's own database.** `gojellyfin import` reads a foreign install's `jellyfin.db`, which sits under `<config>/data` beside `library.db`. `Users`, `Permissions` and `Devices` came through the 10.11 rewrite unchanged, so one set of queries reads a 10.10 and a 10.11 install; the items and the watch state did not, and nothing here reads them. A password is a PHC string with the salt and the hash in **hex** rather than base64 — `$PBKDF2-SHA512$iterations=<n>$<salt>$<hash>` is PBKDF2-HMAC-SHA512 truncated to 64 bytes, and the legacy `$PBKDF2` is SHA1 and 32 — so `auth.Verify` derives against the iteration count the hash itself carries, and a bcrypt hash is told apart by its leading `$2`. `Permissions.Kind` stores `PermissionKind` by ordinal, and `Users.SubtitleMode` and `SyncPlayAccess` store their enums the same way; all three are spelled identically either side, which is why a position in a list is the whole of the translation. EF Core writes a `DateTime` into a TEXT column with no zone and every one Jellyfin stores is UTC, so a value matching none of the layouts is an error rather than a zero time that reads as a device last seen in year one. `Devices.IsActive` is cleared on sign out, so an inactive row is a token already revoked there — and an active one never expires, which is the whole reason `--active-days` exists.

**A matched username is not a matched person.** The import creates a user it cannot find here and otherwise leaves the local account exactly as it is, which means the two accounts sharing a name have not been shown to be the same person. So the devices of a user who was already here are skipped and reported rather than carried: attaching a foreign token to a local account signs that device in as whoever holds the name here, and a restricted user in the source landing on an administrator here is the case that costs the server. `sessions.Import` upserts a session on its access token but never clears `revoked_at`, for the same reason in the other direction — a token the operator killed here stays dead through the next run.
