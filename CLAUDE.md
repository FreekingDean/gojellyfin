...
- **`internal/{auth,sessions,users,items,libraries,config,setup}` — domains.** Behaviour over the ent models, exposing a `Service` built with `New(client *store.Client)`. `setup` is the one that composes rather than queries — `New(config, users)` — because "is this server set up" is a predicate over two domains and belongs to neither. **A domain package must never import `internal/server/api` or `internal/http/middleware`.** That invariant is what the layout rests on; check it with `grep -rl 'server/api\|http/middleware' internal/<domain>/`, which must come back empty. `auth` owns hashing, tokens and request identity; `sessions` owns the active session and device rows, which are Jellyfin sessions rather than login state.
...
A fresh database is set up through the startup wizard in `internal/server/startup`, which creates the first administrator. Its endpoints answer without a token and every one of them refuses with 403 once the wizard is done, because a wizard that still answers on a live server hands it to whoever asks. `setup.Completed` is the gate: the `IsStartupWizardCompleted` flag stored under the `system` configuration key, defaulting to whether the database holds a user at all, so an install that predates the wizard is closed rather than open. It reads `(false, err)` on a failed read, which is only safe because every caller checks the error first. The `setup` domain owns that key's fields as raw JSON and writes them back one at a time, so storing the flag cannot drop the settings it does not model. `GetSystemInfo` and `GetPublicSystemInfo` report the gate as `StartupWizardCompleted`, which is how `jellyfin-web` decides to redirect into the wizard. `middleware.Auth` skips authentication on the same condition for all 27 operations the spec marks with a `FirstTimeSetup` scope, generated into `api.FirstTimeSetupOperations` from the vendored spec so that widening the set can only happen as a reviewable diff. That is what lets the wizard reach the library, environment and localization steps a fresh install needs. Only the seven `Startup` operations refuse of their own accord; the other twenty just fall back to normal token auth once the flag is set, and a gate that cannot be read is an error rather than an open wizard.

`cmd/tasks/adduser` still creates a user from the command line. It reads the password from stdin rather than a flag, which keeps it out of the shell history and the process list. One-off jobs that need the domain services rather than a running server belong beside it under `cmd/tasks`.
