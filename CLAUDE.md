...
Two files are only versions of each other if they are the same cut, and runtime is the cheap test: `items.sameCut` groups them within two minutes or across the 25/24 PAL speedup, and a divergent runtime takes a key of its own (`…:cut186`) rather than becoming a version of the film already there. The first cut scanned holds the bare key, so which of two cuts is bare depends on walk order — deterministic within a library, but not stable if the bare one is later removed and re-added (#517).

That ordering is why the scan probes the file **before** it decides which item the file belongs to: the runtime is the input to that decision. `probed_at` and `date_modified` therefore live on the source, so an unchanged file still costs no ffprobe.
