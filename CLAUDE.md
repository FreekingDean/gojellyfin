...
**Local artwork wins a kind, and in both orders.** An operator who put a poster beside the file made an explicit choice, and the scan and the identify job run on different schedules, so neither is reliably first. `items.SaveImage` deletes a `Remote` row in the slot before it writes, and the download skips a kind a `Local` row already holds. `TestService_SaveImage` and `TestService_SaveDownloadedImage` write each order directly and `TestService_IdentifyItems_Artwork` drives both through the job.

**The key is the content and so is the tag.** A downloaded image is stored under `items/<id>/<kind>/<sha1 of the url><ext>`, and that same hash is the row's `tag`. A run already holding that key downloads nothing, which is the whole of the re-download predicate; a poster TMDB has changed is a different url, so a different key, and `SaveDownloadedImage` updates the row in place and deletes the blob it replaced. Writing the tag from the url rather than leaving the row's original in place is what keeps `?tag=` busting client caches (#623).

A download is bounded by a thirty second client timeout and a sixteen megabyte read, so a CDN that hangs costs one image rather than the activity. A 404, a timeout and a store that refuses are all logged and stepped past, per image — the loop heartbeats and checks `ctx.Err()` between them, the same as the item loop above it.
...
**`internal/artwork` is what anything else names; `internal/artwork/postgres` is one implementation of it.** `Store` is `Put`, `Open` and `Delete` over a string key, `artwork.New` is the only place that picks who answers, and `TestBackendStaysBehindArtwork` fails on an import of the backend from anywhere else — so an object store is a second `New` case rather than a rewrite. `Open` answers a miss with `false` rather than a sentinel, for the same reason `metadata.Provider` does: a sentinel in `artwork` would make the backend import its consumer.

**The bytes are in Postgres because the database is already shared across every replica**, which is the whole reason pod-local disk cannot hold them; it also needs no bucket, no credentials and no new dependency, and keeps a backup atomic so a row and its bytes cannot drift apart. The cost is backup size and a connection per image request (#625). They live in `image_blobs` keyed by the storage key rather than in a column on `image`, because item queries and DTO building read image rows constantly and must never drag a poster along; nothing joins the two, and the row's `path` is the key, which is what lets `server/image` keep deriving the content type from its extension.

**`image.source` is the ownership split.** `Local` means a file the scan found beside the media and the row points at it in place; `Remote` means bytes in the store. `items.DeleteImagesNotInPaths` sweeps `Local` only, so the scan deletes what the scan wrote and a downloaded poster survives a walk that never saw a file for it. `internal/metadata` is what writes a `Remote` row, and Metadata providers is where what it stores under is written down. Nothing collects the bytes an image row leaves behind (#626); the one case that is handled is a downloaded poster replacing the downloaded poster before it, because that write is holding the old row and can read the key off it.
