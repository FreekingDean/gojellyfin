...
`make e2e` is the one test that boots the server. `cmd/gojellyfin/e2e_test.go` sits behind a `//go:build e2e` tag so `make test` never picks it up, creates a database of its own and drops it on the way out, runs `migrateCommand` and `addUserCommand` in process — piping the password through a swapped `os.Stdin`, because that command takes it no other way — seeds a library, and then starts the real `serverModules` graph and drives it over HTTP: public system info, a refused anonymous request, a refused wrong password, a login, the user behind the token, the library as a view, its items through the `/Users/{userId}/Items` alias, an item opened and favourited and re-read, and the websocket greeting. It takes about four seconds and CI runs it on every PR as a step in the existing `test` job, which already has the postgres and the atlas it needs.

It picks its port by binding `:0` and reading the number back into `HTTP_PORT`, so it can never take `:8081` from a running `air`.

**There is no browser in it, and that is the finding rather than an omission.** `jellyfin-web` is not published anywhere a test can fetch it: no npm package, no built assets on its releases, and the only place the built client exists is inside the all-in-one `jellyfin/jellyfin` image (see the comment in `deploy/web-deployment.yaml`). Driving the real client would mean pulling that image on every run and adding a browser driver, which is a lot of machinery to assert what the HTTP flows above already assert. The API is the contract the client talks to, so the smoke test is written against the API.
