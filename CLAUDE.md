...
`internal/jobs` is the client, the worker and the registry the `ScheduledTasks` API is served from, and it is the only package that names the engine — a job is written against `jobs.Job`, `Context`, `Step` and `Child`, which `TestNothingElseImportsTheWorkflowEngine` enforces. A package declares what it runs beside the code that implements it: a `Job` names its steps and its children, and `internal/scanner/fx.go` registers it, so the worker command lists no workflows of its own.
...
The library scan is a walk and a fan out, split because they are different sizes. `ScanLibrary` walks one library, writes its titles and its files and sweeps what has gone, and there is one of those per library, so they fan out flat. The probes are per file and there are thousands, so they fan out through `ProbeChunk` children of a hundred each: a workflow replays its whole history, and a flat fan out of thousands of activities is what Temporal's own guidance caps at a few hundred. The parent's history holds a handful of children, and each child's holds its own hundred probes.
