...
So is `jellyfin-web`. `internal/http/web` serves the client at `/web/` and redirects `/` to it, which is the upstream URL shape and the reason the browser never makes a cross-origin call — the CORS allowlist in `middleware.HttpCORS` exists for clients hosted elsewhere, not for the one the server ships. `/web/*` is a catch-all, so it registers **last**, after the generated `/web/ConfigurationPage` and `/web/ConfigurationPages`; `TestApiRoutesUnderWebWin` fails if that order is reversed. Anything under `/web/` that is not a file on disk answers `index.html`, because the client is a single-page app. `http.ServeContent` does the serving rather than `http.ServeFile`, which redirects `/web/index.html` to `./` and so would bounce the one path clients ask for by name.

`JELLYFIN_WEB_ROOT` is read with `os.LookupEnv` rather than `os.Getenv`: unset means the `/usr/share/jellyfin/web` the Debian package installs to, and set-but-empty is the off switch for an operator hosting the client elsewhere. A root with no `index.html` logs once and registers no routes at all, so a source checkout with no client fetched still boots and still serves the API.

The client is not vendored and not embedded. `jellyfin-web` publishes no built release artifact — its GitHub releases carry no assets, and the `jellyfin-web` npm package is a 78KB metadata-only publish — so the `jellyfin-web_<version>_all.deb` on `repo.jellyfin.org` is the only official build there is. A `dpkg-deb -x` stage in the `Dockerfile` unpacks it, which keeps 59MB of hashed bundles out of the repo and out of `go build`, and keeps the client version a one-line `ARG JELLYFIN_WEB_VERSION` bump rather than a commit full of binaries. `go:embed` was the alternative and fails the same test twice: it would put those bundles in git and would make a checkout without them fail to compile.
