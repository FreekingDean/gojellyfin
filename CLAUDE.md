...
**A client is handed one url and no choice of path.** `PlaybackInfo` answers every video source with `SupportsDirectPlay` and `SupportsDirectStream` false, `SupportsTranscoding` true, and a `TranscodingUrl` this server generated, so `jellyfin-web` never builds a stream url of its own and never picks between two of them. Where that url leads is decided from the `DeviceProfile` the client posted, and a `DirectPlayProfile` is a triple, so **which of the three failed is the whole of the decision**. `mediainfo.plan` answers it as a ladder, cheapest first:

1. the container and both codecs are declared — the url names the source's own container and codecs and the handler serves the file, byte ranges and all;
2. the codecs are declared and the container is not — the url names a container the client declared that `internal/transcode` can write, and both streams are copied into it;
3. the audio codec is not declared — the same mux with `-c:a` set to what that container carries, and the picture still copied;
4. the video codec is not declared — refused. Encoding a picture is #481, so `plan` returns `refused`, the source is answered with all three flags false, and a response where no source survives carries `ErrorCode: NoCompatibleStream`.
