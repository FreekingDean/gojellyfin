...
An upsert whose conflict does nothing returns no row, which ent hands back as `sql.ErrNoRows`, so both image writes read that as the conflict it is rather than as a failure. Until they did, a rescan writing a poster already on the item failed the scan step, and no test had ever scanned one poster twice.
