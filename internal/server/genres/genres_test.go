...
func TestServer_GetGenre(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("answers a genre that exists", func(t *testing.T) {
		response, err := server.GetGenre(ctx, api.GetGenreRequestObject{GenreName: name})
		if err != nil {
			t.Fatalf("failed to get the genre: %v", err)
		}

		result, ok := response.(api.GetGenre200JSONResponse)
		if !ok {
			t.Fatalf("response = %T, want api.GetGenre200JSONResponse", response)
		}
		if result.Name == nil || *result.Name != name {
			t.Errorf("name = %v, want %s", result.Name, name)
		}
		if result.Type == nil || *result.Type != api.BaseItemKindGenre {
			t.Errorf("type = %v, want %s", result.Type, api.BaseItemKindGenre)
		}
	})

	t.Run("refuses a name that matches nothing", func(t *testing.T) {
		response, err := server.GetGenre(ctx, api.GetGenreRequestObject{GenreName: name + "-absent"})
		if err != nil {
			t.Fatalf("a missing genre returned an error rather than a response: %v", err)
		}
		if _, ok := response.(api.GetGenre403Response); !ok {
			t.Errorf("response = %T, want api.GetGenre403Response", response)
		}
	})
}
