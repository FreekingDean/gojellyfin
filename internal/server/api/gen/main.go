...
func main() {
	...
	writeOperationPolicies(f.Name.Name, methods)
	writeQueryParameters(f.Name.Name)
	writeJellyfinVersion()
}
...
const JellyfinVersion = %q
`, versionPkg, doc.Info.Version)
...
func writeOperationPolicies(pkg string, methods *ast.FieldList) {
	...
	for _, item := range doc.Paths {
		for method, raw := range item {
			...
			for _, scheme := range op.Security {
				for _, granted := range scheme {
					scopes = append(scopes, granted...)
				}
			}
			sort.Strings(scopes)
			policies[name] = scopes
		}
	}
	...
	for _, name := range methodNames {
		...
		if _, ok := public[name]; !ok {
			log.Fatalf("%s has no security in the spec and is not public: %s", iface, name)
		}
	}
...
`, pkg)
	...
	for _, name := range names {
		quoted := make([]string, 0, len(policies[name]))
		for _, scope := range policies[name] {
